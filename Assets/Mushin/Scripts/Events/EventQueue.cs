...
namespace Mushin.Scripts.Events
{
    public class EventQueue : MonoBehaviour
    {
        ...
        {
            if (!_eventIdToObservers.TryGetValue(eventId, out var observers))
            {
        ...
        public void Unsubscribe(EventIds eventId, IEventObserver observer)
        {
            ...
            {
                var removeData = new RemoveData(eventId, observer);
        ...
        private void DoUnsubscribe(EventIds eventId, IEventObserver observer)
        {
            _eventIdToObservers[eventId].Remove(observer);
        }
        ...
        private void ProcessEvent(EventData eventData)
        {
            ...
            if (_eventIdToObservers.TryGetValue(eventData.EventId, out var eventObservers))
            {
                ...
                {
                    observer?.ProcessEvents(eventData);
                }
            }
        ...
        {
            foreach (var removeData in _pendingObserversToUnsubscribe)
            {
